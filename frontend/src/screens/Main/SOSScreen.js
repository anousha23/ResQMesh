...
export default function SOSScreen() {
  ...
  const sendVoiceSos = async () => {
    ...
    const transcriptText = "I am trapped inside a building on the second floor, two of us can't walk";
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("Offline NLU Incident Classification:", data);
      }
    } catch (err) {
      console.log("Using offline direct relay fallback:", err);
    }
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.centerContainer}>
          ...
          {classifiedData && (
            <View style={{ marginTop: 20, padding: 16, backgroundColor: '#1e1e1e', borderRadius: 12, width: '100%', borderWidth: 1, borderColor: '#333' }}>
              <Text style={{ color: '#ef4444', fontWeight: 'bold', fontSize: 16, marginBottom: 4 }}>
                NLU CLASSIFICATION: {classifiedData.incident_type?.toUpperCase()}
              </Text>
              <Text style={{ color: '#fff', fontSize: 14, marginVertical: 2 }}>
                Severity: {classifiedData.severity?.level?.toUpperCase()} (Score: {classifiedData.severity?.score}/10)
              </Text>
              <Text style={{ color: '#3b82f6', fontSize: 14, marginVertical: 2 }}>
                Priority Resource: {classifiedData.resource_needs?.priority_resource}
              </Text>
              <Text style={{ color: '#aaa', fontSize: 14, marginVertical: 2 }}>
                Mobility: {classifiedData.victims?.mobility_status || 'Normal'}
              </Text>
            </View>
          )}

          <Button title="Done" onPress={() => { setVoiceState('IDLE'); setClassifiedData(null); }} style={{marginTop: 32, width: '100%'}} />
        </View>
      </SafeAreaView>
    );
  }
